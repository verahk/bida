...
To sample from the network, we apply the `bida::sample_data_from_bn` function. The function re-samples data sets until at least 2 levels of every variable is observed (required for `BiDAG::partitionMCMC`). 
```{r}
...
### Apply BIDA-method
Given a sample of DAGs and a class of backdoor adjustmnent sets, we apply`bida:::bida_sample` to estimate the posterior over the intervention distributions for each cause-effect pair.
...
```{r}
fit  <- bida_sample(data, dags, type = "categorical", adjsets = "pa") 
```


### Sampling from the posterior distribution
Samples from the posterior distribution is obtained trhough Monte Carlo sampling.
