                      ...
                      addTimeStamp = FALSE)
{
  ...
  if (length(names_from) == 2 & length(values_from) == 1){
    new_cols <- as.character(unique(df[[names_from[1]]]))
    df <- tidyr::pivot_wider(df, values_from = values_from, names_from = names_from[1])
    names_from  <- names_from[-1]
    values_from <- new_cols
  }
