...
for (n in c(10, 20)) {
  for (k in c(2, 4)) {
    ...
    for (name in names(ylabs)) {
      ...
      # plot selection
      plot_sub <- df  %>%
        filter(!name == "full CPT", edgepf == "2") %>%
        plot_boxplot(x, y, facets, fill = fill, ylab = ylabs[[name]])

      # plot everything
      caption <- paste0("The box plot shows the distribution over ", n_sim_runs, " simulations runs.")
      plot_full<- df  %>%
        plot_boxplot(x, y, facets, fill = fill, ylab = ylabs[[name]],
                     caption = caption)
      ...
      if (length(dir_out) > 0 ) {
        ...
        ggsave(paste0(dir_out, filename), plot_sub, height = 7, width = 6)

        filename <- paste0("box_plot_full_", sprintf("n%s_k%s", n, k), "_", name, ".png")
        ggsave(paste0(dir_out, filename), plot_full, height = 7, width = 6)
