...
```{r}
here::here("./tests/test_posterior_sample.bida_pair_bdeu.Rmd")
...
for (i in seq_along(samplesizes)) {
  ...
  for (x in seq_n) {
    ...
    for (y in seq_n[-x]) {
      ...
      m    <- posterior_mean(pair)
      res[i, x, y, 1] <- mean( (m-truth$pdo[[x, y]])**2 )
      m    <- posterior_mean(pair, contrasts = list(jsd = JSD))
      res[i, x, y, 2] <- (m-truth$jsd[x, y])**2   
    }
  }
}
...
```{r}
library(ggplot2)
ggplot(df, aes(N, sqrt(pdo), colour = interaction(y))) +
  facet_wrap(x~.) +
  geom_line() +
  ylab("RMSE, posterior mean, intervention probs") + 
  xlab("samplesize") +
