...
if (FALSE) {
  ...
  sim_run <- function(r, file = NULL) {
    ...
    for (N in dnames$N) {
      ...
      smpls <- lapply(fits,
                      function(fit) posterior_sample(fit, 10**3, contrasts = list(jsd = jsd))[!dindx])
      nrows <- sapply(smpls,
                      function(fit) sapply(fit, nrow))
      stopifnot(all(nrows == 1000))
      means <- sapply(smpls,
                      function(smpl) colMeans(do.call(cbind, smpl)))
      diff  <- means-tau
      colnames(diff) <- names(fits)
  ...
  # Asia-network ----
  dfs <- lapply(1:30, function(r) sim_run(r, "./data/asia.rds"))
  df  <- dplyr::bind_rows(dfs, .id = "r")
  df$N <- factor(df$N)
