...
dags <- fit$traceadd$incidence[-seq_len(200)]
```
...
```{r}
fit  <- bida(dags, data, type = "categorical", adjset = "pa") 
```
...
for (j in 2:4) {
  tau <- posterior_sample(fit[[1, j]], 10**3, contrsasts = list(jsd))
...
```{r}
posterior_mean(fit, contrasts = list(jsd))
```
